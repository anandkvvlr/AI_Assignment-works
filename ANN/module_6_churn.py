...
# load data set
df = pd.read_csv('F:/AI assignment/AI assignment/Module 6/A6_Datasets/Churn_Modelling.csv')
...
# drop column which have no inferances
...
# statistics on the train dataset to make sure it is in good shape
train_stats = train_dataset.describe()
train_stats.pop("Exited")
train_stats = train_stats.transpose()
train_stats

#class from each dataset assigning on new variable
train_labels = train_dataset.pop('Exited')
test_labels = test_dataset.pop('Exited')
valid_labels = valid_dataset.pop('Exited')
...
#fit the model
model = model.fit(x_train,y_train,epochs=30,batch_size=32,validation_data=(x_valid, y_valid))

history_dict = model.history
history_dict.keys()  # values in history
